...
/* Return true if instruction is some kind of jump to subroutine
   Used to detect skip over */
...
int insn_size(unsigned short addr)
{
	...
	else if (cputype == M6809)
	{
		/* M6809 */
		if (mem[addr] == 0x17) return 3; // LBSR
	...
	{
		/* M6800 */
		if (mem[addr] == 0xAD) return 2; /* JSR indexed */
		else return 0;
	}
}
...
unsigned char read_flags()
{
	if (cputype == M6809)
		...
		return (c_flag + (v_flag << 1) + (z_flag << 2) + (n_flag << 3) + (i_flag << 4) + (h_flag << 5) + 0xC0);
...
void m6809_write_ireg(int r, unsigned short data)
{
	switch (r)
	{
...
unsigned short m6809_ireg(int r)
{
...
struct trace_entry
{
	...
	unsigned char accb;
	unsigned short ix;
	unsigned short iy; /* 6809 */
	unsigned short up; /* 6809 */
	unsigned short sp;
	unsigned short pc; /* Address of instruction */
	unsigned char dp; /* 6809 */
	unsigned char cc;
...
int m6809_tfr_get(unsigned short *rtn, int r)
{
	switch (r) {
...
int m6809_tfr_put(int r, unsigned short val)
{
	switch (r) {
...
int m6809_ind(unsigned short *rtn)
{
...
extern char *m6809_regname[];
int m6809_tfr_name(char **name, int r);
void m6809_show_list(int us, char *right, unsigned char cb);

/* Show indexed */

int m6809_show_ind(struct trace_entry *t, int *x, char *buf, char *right)
{
	...
	*x = *x + 1;
	...
	{ /* 5-bit signed offset */
		if (ind) {
			int v = -16 + o;
			sprintf(right, "%d,%s", v, m6809_regname[r]);
			return 0;
		} else {
			sprintf(right, "%d,%s", o, m6809_regname[r]);
			return 0;
		}
	}
	else
	{
		switch (o)
		{
			...
			{
				if (ind)
					sprintf(right, "[,%s+]", m6809_regname[r]);
				else
					sprintf(right, ",%s+", m6809_regname[r]);
				return 0;
			}
			...
			{
				if (ind)
					sprintf(right, "[B,%s]", m6809_regname[r]);
				else
					sprintf(right, "B,%s", m6809_regname[r]);
				return 0;
			}
			...
			{
				if (ind)
					sprintf(right, "[A,%s]", m6809_regname[r]);
				else
					sprintf(right, "A,%s", m6809_regname[r]);
				return 0;
			}
			case 0x08: /* Offset8,R */
			{
				...
				*x = *x + 1;
				if (ind)
					sprintf(right, "[%d,%s]", (signed short)v, m6809_regname[r]);
				else
					sprintf(right, "%d,%s", (signed short)v, m6809_regname[r]);
				return 0;
			}
			...
			{
				if (ind)
					sprintf(right, "[D,%s]", m6809_regname[r]);
				else
					sprintf(right, "D,%s", m6809_regname[r]);
				return 0;
			}
...
void m6809_show_trace(int insn_no, struct trace_entry *t)
{
	int x;
        char *fact_label;
        char *fact_comment;
	...
	if (opcode & 0x80) { /* R,M operations */
		...
		switch (opcode & 0x30) {
			case 0x00: {
				...
				if (m6809_show_ind(t, &x, buf, right))
					goto invalid;
				memop = 1;
				break;
			} case 0x30: {
		...
		switch(opcode) {
			case 0x12: { /* NOP */
				...
				insn = "EXG";
				sprintf(buf + strlen(buf), " %2.2X", t->insn[x]);
				if (m6809_tfr_name(&n, t->insn[x] >> 4))
					goto invalid;
				strcat(right, n);
				if (m6809_tfr_name(&n, t->insn[x] & 0xF))
					goto invalid;
				x++;
				strcat(right, ",");
				strcat(right, n);
				...
				insn = "PSHS";
				m6809_show_list(0, right, t->insn[x++]);
				break;
			} case 0x35: { /* PULS */
				insn = "PULS";
				m6809_show_list(0, right, t->insn[x++]);
				...
				insn = "PSHU";
				m6809_show_list(1, right, t->insn[x++]);
				break;
			} case 0x37: { /* PULU */
				insn = "PULU";
				m6809_show_list(1, right, t->insn[x++]);
...
void m6800_show_trace(int insn_no, struct trace_entry *t)
{
        char *fact_label;
        char *fact_comment;
        char *ea_label;
...
{
	if (cputype == M6809)
		m6809_show_trace(insn_no, t);
	else
		m6800_show_trace(insn_no, t);
}
				...
				if (!(opcode & 0xF0)) {
                	...
                	switch(opcode) {
				        ...
				        break;
                			...
                			unsigned short tmp;
                			unsigned short tmp1;
                			if (m6809_tfr_get(&tmp, cb & 0x0F))
                				goto invalid;
                			if (m6809_tfr_get(&tmp1, cb >> 4))
                				goto invalid;
                			m6809_tfr_put((cb & 0x0F), tmp1);
                			m6809_tfr_put(cb >> 4, tmp);
                			break;
                		} case 0x1F: { /* TFR */
                			unsigned char cb = fetch();
                			unsigned short tmp1;
					...
					if (m6809_tfr_put((cb & 0x0F), tmp1))
						goto invalid;
                			...
                			unsigned char cb = fetch();
                			if (cb & 0x01) write_flags(pulls());
                			if (cb & 0x02) acca = pulls();
                			if (cb & 0x04) accb = pulls();
                			if (cb & 0x08) dp = pulls();
                			if (cb & 0x10) ix = pulls2();
                			if (cb & 0x20) iy = pulls2();
                			if (cb & 0x40) up = pulls2();
