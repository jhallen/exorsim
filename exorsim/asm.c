/*	6800 and 6809 online assembler
 *	Copyright
...
enum {
        LREL=1,	/* 6809 Long branch, 16-bit offset */
        REL=2,	/* 6800+6809 Relative branch, 8-bit offset */
        IMM8=3,
        IMM16=4,
        TFR=5, /* 6809 */
        IND=6,	/* 6809 Indexed */
        SLIST=7, /* 6809 */
        ULIST=8, /* 6809 */
        DIR=9,	/* 6800+6809 Direct */
        EXT=10,	/* 6800+6809 Extended */
        ...
        NONE=17, /* No operand */
        SETDP=18, /* 6809 setdp pseudo-op */
        IDX=19, /* 6800 indexed */
        RM=20, /* 6800 Register-Memory */
        RMW=21, /* 6800 Read-modify-write */
        ACC=22, /* 6800 Accumulator needed */
        ACC1=23, /* 6800 Accumulator optional */
        ACCB=24, /* 6800 We need A or B: add 0x01 for B */
};
...
struct { char *insn; int opcode; int type; } insn_table_6800[] =
{
        { "lda", 0x86, ACC1 },
        { "sta", 0x87, ACC1 },
        { "ora", 0x8a, ACC1 },
...
unsigned short assemble6809(unsigned char *mem, unsigned short addr, char *buf)
{
        char str[80];
        ...
        /* Lookup instruction */
        insn = 0;
        for (x = 0; insn_table_6809[x].insn; ++x)
                if (!jstricmp(insn_table_6809[x].insn, str)) {
                        insn = &insn_table_6809[x];
                        break;
                }
...
unsigned short assemble6800(unsigned char *mem, unsigned short addr, char *buf)
{
        char str[80];
        unsigned short label_addr = addr;
        struct symbol *label_sy;
        ...
        /* Lookup instruction */
        for (x = 0; insn_table_6800[x].insn; ++x)
                if (!jstricmp(insn_table_6800[x].insn, str)) {
                        opcode = insn_table_6800[x].opcode;
                        type = insn_table_6800[x].type;
                        break;
                }

        if (!insn_table_6800[x].insn) {
                printf("Huh?\n");
                return addr;
        }
...
{
        if (cputype == M6809)
                return assemble6809(mem, addr, buf);
        else
                return assemble6800(mem, addr, buf);
}
