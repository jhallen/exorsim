...
void unasm_line_6809(unsigned char *mem, unsigned short *at_pc, char *outbuf, int *at_target, int flag)
{
	unsigned short pc;
	struct fact *fact;
	char buf[150];
	...
	if (opcode & 0x80) { /* R,M operations */
		...
		switch(opcode) {
			case 0x12: { /* NOP */
				...
				insn = "EXG";
				sprintf(buf + strlen(buf), " %2.2X", mem[pc]);
				if (m6809_tfr_name(&n, mem[pc] >> 4))
					goto invalid;
				strcat(right, n);
				if (m6809_tfr_name(&n, mem[pc] & 0xF))
					goto invalid;
				pc++;
				strcat(right, ",");
				strcat(right, n);
				...
				insn = "PSHS";
				m6809_show_list(0, right, mem[pc++]);
				break;
			} case 0x35: { /* PULS */
				insn = "PULS";
				m6809_show_list(0, right, mem[pc++]);
				...
				insn = "PSHU";
				m6809_show_list(1, right, mem[pc++]);
				break;
			} case 0x37: { /* PULU */
				insn = "PULU";
				m6809_show_list(1, right, mem[pc++]);
...
void unasm_line_6800(unsigned char *mem, unsigned short *at_pc, char *outbuf, int *at_target, int flag)
{
	unsigned short pc;
	struct fact *fact;
	char *insn;
...
{
	if (cputype == M6809)
		unasm_line_6809(mem, at_pc, outbuf, at_target, flag);
	else
		unasm_line_6800(mem, at_pc, outbuf, at_target, flag);
}
