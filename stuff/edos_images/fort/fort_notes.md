...
- [`L2575`](fort.asm#L2804) is the top-level resident driver.
- [`L0899`](fort.asm#L177) prints the startup banner.
- [`L097C`](fort.asm#L206) performs cold-start initialization.
- [`L26D0`](fort.asm#L2915) reads the next source line and resets per-line state.
...
- The keyword table starts at [`$01EC`](fort.asm#L95).
- Per-keyword metadata lives at [`$02D1`](fort.asm#L120).
- [`L0B83`](fort.asm#L458) is the statement classifier front door.
- [`L111C`](fort.asm#L1127), [`L1205`](fort.asm#L1177), and [`L121E`](fort.asm#L1192) walk descriptor tables derived from the statement metadata.
...
- [`STMT_DISPATCH`](fort.asm#L1254) at `L1295` is the main statement-handler jump table.
- The declaration/data side appears to include:
  - [`STMT_DECLARE_OR_REFERENCE`](fort.asm#L1270)
  - [`STMT_DECLARATION_SETUP`](fort.asm#L1300)
  - [`STMT_DIMENSION_STORE_BOUND`](fort.asm#L1349)
  - [`STMT_DIMENSION_OR_COMMON_FINALIZE`](fort.asm#L1366)
  - [`STMT_DATA_OR_COMMON_PREP`](fort.asm#L1426)
  - [`STMT_DATA_OR_COMMON_EMIT_FIELDS`](fort.asm#L1453)
- The transfer/codegen side appears to include:
  - [`STMT_TRANSFER_FIXUP_MERGE`](fort.asm#L1503)
  - [`STMT_TRANSFER_CODEGEN_DISPATCH`](fort.asm#L1766)
- The source-I/O side appears to include:
  - [`STMT_SOURCE_IO_OR_TRANSFER_DISPATCH`](fort.asm#L2682)
...
Inside [`STMT_TRANSFER_CODEGEN_DISPATCH`](fort.asm#L1838), the subentries are now more usable:

- [`TRANSFER_EMIT_ASSIGNED_GOTO`](fort.asm#L1853) is now the best assigned-`GOTO` candidate. The emitted `FE .. 6E 00` pattern looks like “load X from variable; jump indirect through X”.
- [`TRANSFER_LIST_INIT`](fort.asm#L1868) and [`TRANSFER_LIST_APPEND_SYMBOL`](fort.asm#L1874) manage a temporary target list at `$0390`.
- [`TRANSFER_EMIT_COMPUTED_GOTO`](fort.asm#L1896) is the strongest computed-`GOTO` candidate so far. It emits an inline dispatcher skeleton and then serializes the accumulated target list.
- [`TRANSFER_RESOLVE_AND_EMIT_TARGET`](fort.asm#L1958) and [`TRANSFER_DEFINE_AND_EMIT_TARGET`](fort.asm#L2024) look like direct-target helpers for plain branch/call-style forms.
- [`TRANSFER_EMIT_RETURN`](fort.asm#L2065) is now the clearest `RETURN`-style emitter in the transfer family, because its fixed template is the word `0x3900` (`RTS` plus terminator/padding in this object format).
- [`TRANSFER_ALLOC_LOCAL_TARGET`](fort.asm#L2069), [`TRANSFER_LIST_APPEND_EXPR_DESC`](fort.asm#L2095), and [`TRANSFER_LIST_SERIALIZE`](fort.asm#L2116) reinforce the idea that this family handles transfer statements with explicit target lists.

### 4. Expression parser

- [`L1EF1`](fort.asm#L2318) is the clearest expression-parser core.
- Operator precedence/class data is stored at [`$1EC9`](fort.asm#L2298).
- Temporary expression descriptors are pushed/popped with:
  - [`L1DD5`](fort.asm#L2212)
  - [`L1E8A`](fort.asm#L2291)
- Operator/action dispatch goes through:
  - [`L194D`](fort.asm#L2042)
...
- [`DIAG_FORMAT`](fort.asm#L912) at `L0EEE` is the higher-level diagnostic formatter.
- [`DIAG_BUILD_AND_PRINT`](fort.asm#L930) at `L0F08` builds and prints the message text.
- [`L0D7F`](fort.asm#L662) emits error messages using the current line/listing context.
...
The fixed pointer block at [`$1277`](fort.asm#L1400) is now clearer:
...
The keyword metadata table at [`$02D1`](fort.asm#L120) is also a little clearer now:
...
- [`STMT_SOURCE_IO_OR_TRANSFER_DISPATCH`](fort.asm#L2682) at `L22C2` is the current best candidate for `READ`/`PRINT`/`WRITE`/`FORMAT`-family handling.
- Notable subentries:
  - [`IO_ITEM_EMIT`](fort.asm#L2705)
  - [`IO_CONTROL_OR_FORMAT_ITEM`](fort.asm#L2756)
  - [`IO_TEMPLATE_ADVANCE6`](fort.asm#L2803)
  - [`IO_TEMPLATE_PRESERVE_PTR`](fort.asm#L2819)
  - [`IO_TEMPLATE_08_09`](fort.asm#L2837)
...
- [`CODEGEN_TEMPLATE_DISPATCH`](fort.asm#L2864) at `L23D1`
- [`OBJECT_FRAGMENT_DISPATCH`](fort.asm#L2969) at `L2716`
- [`L28C7`](fort.asm#L3250) as the main object-record emitter
- [`L2A40`](fort.asm#L3439) / [`L2A5E`](fort.asm#L3452) for record buffering
- [`L2C42`](fort.asm#L3587) for flush/finalization

Useful template subentries include:

- [`TEMPLATE_RESET_STATE`](fort.asm#L2882)
- [`TEMPLATE_EMIT_KEYWORD_TRAILER`](fort.asm#L2899)
- [`TEMPLATE_PACK_AND_EMIT`](fort.asm#L2922)
- [`TEMPLATE_KEYWORD_SCAN`](fort.asm#L2948)
- [`TEMPLATE_EMIT_TEXT_REF`](fort.asm#L2967)
- [`TEMPLATE_EMIT_FIXUP_RECORD`](fort.asm#L2989)
...
- [`L288A`](fort.asm#L3528) outputs a printable/object record as:
  - one record-type byte in `A`
  - `LIST_REC_LEN` bytes from `LIST_REC_PTR`
  - a two's-complement checksum
  - `CR`, `LF`, and a trailing NUL
- [`L2A40`](fort.asm#L3737) starts a raw payload buffer at `$066A`.
- [`L2A5E`](fort.asm#L3749) appends contiguous payload bytes.
- [`L2AB2`](fort.asm#L3795) and neighbors collect relocation metadata as packed two-bit classes plus short descriptor bytes.
- [`L2C42`](fort.asm#L4012) flushes the raw payload and then any relocation side-record.
- [`L2C67`](fort.asm#L4028) emits the module prologue, including visible record types `0x36`, `0x32`, `0x33`, and optionally `0x34`, plus the default module text around `$0964` (`"30      3$MAIN  "`).
- [`COPY_FRAGMENT_BYTES`](fort.asm#L4235) at `L2E17` copies small fragment descriptors from the temporary stack into record buffers, which helps explain how `OBJECT_FRAGMENT_DISPATCH` materializes its mini-templates.
...
- [`STMT_END_AND_SYMBOL_LIST`](fort.asm#L1523) at `L14D6` appears to produce the end-of-compilation summary.
...
- The fixed pointer/constant block around [`$1277`](fort.asm#L1400) now has working labels and comments in the assembly listing.
